...
def detect_encoding(file_path):
    with open(file_path, "rb") as f:
        result = chardet.detect(
            f.read(10000)
        )  # Read the first 10,000 bytes for detection
        ...
        for line in infile:
            ...
            if len(parts) == 2:
                ...
                if cleaned_word:
                    # Remove extra spaces and unwanted symbols from the word
                    cleaned_word = re.sub(
                        r"[^\w\s]", "", cleaned_word.strip()
                    )  # Removes punctuation except for alphanumeric and spaces
                    new_line = number + ", " + cleaned_word + "\n"
                    outfile.write(new_line)


if __name__ == "__main__":
    input_file = "word_counts_novels.txt"  # Replace with your input file path
    output_file = "word_counts_novels_clean.txt"  # Replace with your output file path
    process_file(input_file, output_file)
